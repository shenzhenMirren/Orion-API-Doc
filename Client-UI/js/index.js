...
function loadProject(data) {
    try {
        var top = '';
        //title begin
        top += '<h2 style="color: white" class="margin-bottom-0">';
        if (data.name != null) {
            top += data.name;
        }
        if (data.versions != null) {
            top += ' <span class="badge font-size-14px border-1px-solid-orange-bc-transparent"> ' + data.versions + '</span>';
        }
        top += '</h2>';
        //title end
        //description begin
        if (data.description != null) {
            top += '<div  style="color: white" >' + data.description + '</div>';
        }
        //description end
        //updateTime begen
        if (data.lastTime != null) {
            top += '<div>更新时间: ' + data.lastTime + '</div>';
        }
        //updateTime end
        ...
        if (data.schemes != null && data.schemes.length > 0) {
            main += '<span>请求协议: </span><select id="project_scheme" class="border-1px-solid-ccc">';
        ...
        main += '<div class="margin-top-5px margin-bottom-0">' +
        ...
        if (data.contactName != null) {
            main += '<div>联系人: <span>' + data.contactName + '</span> <div>';
        }
        if (data.contactInfo != null) {
            main += '<div> 联系方式: <span>' + data.contactInfo + '</span><div>';
        }
        //contact end
        //externalDocs begen
        ...
        if (apis != null) {
            for (var i = 0; i < apis.length; i++) {
                ...
                locationData.apis.push({
                    'api': (api.title == null ? api.path : api.title),
                    'apiId': api.apiId
                });
                // 接口信息 begin
                ...
                if (api.parameters != null && api.parameters.length > 0) {
                    ...
                    var txt =
                        ...
                        '<thead>' +
                        '<tr>' +
                        '<th class="text-center" width="13%">是否必填</th>' +
                        '<th width="15%">参数位置</th>' +
                        '<th width="17%">参数类型</th>' +
                        '<th width="20%">参数名称</th>' +
                        '<th>参数描述</th>' +
                ...
                //响应结果 end
                //附加说明 begin
                if (api.additional != null) {
                    var instrs = api.additional;
                    if (instrs.length > 0) {
